...
export async function GET(
  ...
  try {
    ...
    console.log(
      `채널 ${channelId}에 SSE 연결 요청 받음 - 사용자: ${userName}(${userId}) - 요청 URL: ${request.url}`
    );
    ...
    const stream = new ReadableStream({
      start(ctrl) {
        ...
        pingInterval = setInterval(() => {
          try {
            const pingMessage = `data: ${JSON.stringify({
              ...
              connectionCount:
                messageManager.getChannelConnectionCount(channelId),
            })}\n\n`;
        ...
        request.signal.addEventListener("abort", () => {
          console.log(
            `채널 ${channelId} 사용자 ${userName}(${userId}) 연결 종료됨`
          );
          messageManager.removeController(channelId, controller, userId);
          if (pingInterval) {
            clearInterval(pingInterval);
      ...
      cancel() {
        console.log(
          `채널 ${channelId} 사용자 ${userName}(${userId}) 스트림 취소됨`
        );
        messageManager.removeController(channelId, controller, userId);
        if (pingInterval) {
          clearInterval(pingInterval);
...
export async function POST(
  ...
  try {
    ...
    if (message.channelId !== channelId) {
      console.log(
        `메시지 채널 ID(${message.channelId})와 요청 URL 채널 ID(${channelId})가 다릅니다. URL 채널 ID로 덮어씁니다.`
      );
    ...
    console.log(
      `채널 ${channelId}에 메시지 브로드캐스트 완료 - 현재 연결 수: ${messageManager.getChannelConnectionCount(
        channelId
      )}`
    );
